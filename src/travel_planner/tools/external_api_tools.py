...
class BookingFlightTools(Toolkit):
    ...
    def search_flights(
        ...
        try:
            ...
            print(f"✈️  [Booking.com] Searching flights: {origin} ({origin_id}) → {destination} ({dest_id}) on {departure_date}")
            logger.info(f"Searching flights: {origin} ({origin_id}) → {destination} ({dest_id}) on {departure_date}")
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == True:
                    ...
                    print(f"✅ [Booking.com] Found {flight_count} flights for {origin} → {destination}")
                ...
                if response.status_code == 200:
                    ...
                    if data.get("status") and data.get("data"):
                        ...
                        for dest in destinations:
                            if dest.get("code", "").upper() == query_upper and dest.get("type") == "AIRPORT":
                        ...
                        for dest in destinations:
                            if dest.get("type") == "AIRPORT":
                                logger.info(f"Using first airport for {query_upper}: {dest.get('id')}")
                                return dest.get("id")
    ...
    def _format_flights_english(self, data: Dict, origin: str, destination: str, max_results: int) -> str:
...
"""
        ...
        for i, flight in enumerate(flight_offers[:max_results], 1):
            ...
            dep_time_str = departure_time.replace("T", " ") if "T" in departure_time else departure_time
    ...
    def search_hotels(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return self._format_hotels_vietnamese(data, location, check_in, check_out, max_results)
            elif response.status_code == 401:
