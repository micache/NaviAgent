...
class Activity(BaseModel):
    ...
    activity_type: str = Field(..., description="Loại hoạt động (tham quan, ăn uống, mua sắm, di chuyển, ...)")
...
class BudgetCategory(BaseModel):
    ...
    category_name: str = Field(..., description="Tên danh mục (Accommodation, Food, Transport, ...)")
...
class LogisticsInfo(BaseModel):
    ...
    accommodation_suggestions: Optional[List[str]] = Field(None, description="Gợi ý khu vực đặt khách sạn")
...
class TravelPlan(BaseModel):
    ...
    # Metadata
    generated_at: str = Field(default_factory=lambda: datetime.utcnow().isoformat(), description="Thời gian tạo")
