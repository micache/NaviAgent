...
def test_plan_trip(user_id: str = None):
    ...
    print(f"   → Session ID will be auto-generated by API")
    print(f"   → Chat history between agents will be saved automatically")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"\n💡 Session Info:")
            print(f"   User ID: {user_id}")
            print(f"   → Session ID was auto-generated internally")
            print(f"   → Chat history between agents saved to database")
            print(f"   → Check Supabase to see agent conversations")
...
def test_multi_turn_conversation():
    ...
    print(f"\n📋 Test Setup:")
    print(f"   User ID: {user_id}")
    print(f"   → Each request will auto-generate its own session_id")
    print(f"   → Session IDs are used internally for agent chat history")

    # =========================================================================
    # TURN 1: Initial 5-day plan
    # =========================================================================
    print(f"\n{'=' * 80}")
    print("🔄 TURN 1: Initial Planning Request (5 days, 30M budget)")
    print("=" * 80)
    ...
    try:
        ...
        if response_1.status_code == 200:
            ...
            print(f"\n❌ Turn 1 Failed (Status: {response_1.status_code})")
    ...
    # =========================================================================
    print(f"\n{'=' * 80}")
    print("🔄 TURN 2: Second Request (Different plan - 7 days, 40M budget)")
    print("=" * 80)
    print("\n💡 This is a new independent request")
    print("   → Will get its own auto-generated session_id")
    print("   → Agent chat history for this request saved separately")
    ...
    try:
        ...
        if response_2.status_code == 200:
            ...
            if plan_1.get("budget") and plan_2.get("budget"):
                ...
                print(f"   Cost: {cost_1:,.0f} VND → {cost_2:,.0f} VND")
            print(f"   Duration: 5 days → 7 days")
            print(f"   Budget: 30M VND → 40M VND")

            print(f"\n✅ Two Independent Requests Complete!")
            print(f"   → Each request got its own session_id (auto-generated)")
            print(f"   → Agent chat history saved separately for each request")
            print(f"   → Check Supabase to see 2 different sessions")

        else:
            print(f"\n❌ Turn 2 Failed (Status: {response_2.status_code})")
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("\n💡 Session Management:")
    print("   • Users only provide user_id (optional)")
    print("   • API auto-generates session_id for each request")
    print("   • Session stores chat history between agents internally")
    print("   • Each plan request = 1 session with all agent conversations")
    print("   • Check Supabase agent_sessions table to see saved chats")
    print("=" * 80 + "\n")
